...
object GetExportedAuditlogs {

  def getExportedAuditlogs(): Unit = {
    val auditLogExportOperations = new AuditLogExportOperations()
    val response: Option[APIResponse[ResponseHandler]] = auditLogExportOperations.getExportedAuditlogs()
            ...
            apiResponse.getObject match {
              case responseWrapper: ResponseWrapper =>
                ...
                if (auditLogExports != null) {
                  for (auditLogExport <- auditLogExports) {
                    ...
                    if (downloadLinks != null) {
                      for (link <- downloadLinks) {
                        println(s"AuditLogExport DownloadLink: $link")
                      }
                    }
