...
class SuccessResponse extends Model with ActionResponse with FileHandler	{
	private var status:Choice[String] = _
	private var code:Choice[String] = _
	private var duplicateField:Option[String] = None
	private var action:Choice[String] = _
	...
	def getDuplicateField() :Option[String]	={
		return  this.duplicateField
	}

	def setDuplicateField( duplicateField: Option[String]) 	={
		 this.duplicateField = duplicateField
		 this.keyModified("duplicate_field") = 1
	}

	def getAction() :Choice[String]	={
		return  this.action
	}

	def setAction( action: Choice[String]) 	={
		 this.action = action
